...
class ValorantCog(commands.Cog, name='Valorant'):
    ...
    @app_commands.command(description='riotアカウントにログインします(このbotではログイン情報が1日でリセットされます)')
    ...
    @app_commands.command(description='ヴァロラントポイントとレディアナイトポイントを取得します')
    # @dynamic_cooldown(cooldown_5s)
    ...
    @app_commands.command(description='ナイトマーケットを取得します')
    # @dynamic_cooldown(cooldown_5s)
    ...
    @app_commands.command(description='バトルパスの現在のティア数を取得します')
    # @dynamic_cooldown(cooldown_5s)
